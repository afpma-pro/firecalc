...
object CommonInstances:
    ...
    given decoder_TempD: [U: SUnit] => Decoder[TempD[U]] = new Decoder[TempD[U]]:
        def apply(c: HCursor): Result[TempD[U]] =
            for
                v_str <- c.downField("value").as[String]
    ...
    given Decoder[ChimneyHeightAboveRidgeline] =
        deriveDecoderForEnum[ChimneyHeightAboveRidgeline](ChimneyHeightAboveRidgeline.valueOf)
    ...
    given Decoder[HorizontalAngleBetweenChimneyAndAdjacentBuildings] =
        deriveDecoderForEnum[HorizontalAngleBetweenChimneyAndAdjacentBuildings](
            HorizontalAngleBetweenChimneyAndAdjacentBuildings.valueOf
        )
    given Encoder[HorizontalAngleBetweenChimneyAndAdjacentBuildings] =
        deriveEncoderForEnum[HorizontalAngleBetweenChimneyAndAdjacentBuildings]

    // HorizontalDistanceBetweenChimneyAndAdjacentBuildings

    given Decoder[HorizontalDistanceBetweenChimneyAndAdjacentBuildings] =
        deriveDecoderForEnum[HorizontalDistanceBetweenChimneyAndAdjacentBuildings](
            HorizontalDistanceBetweenChimneyAndAdjacentBuildings.valueOf
        )
    given Encoder[HorizontalDistanceBetweenChimneyAndAdjacentBuildings] =
        deriveEncoderForEnum[HorizontalDistanceBetweenChimneyAndAdjacentBuildings]

    // HorizontalDistanceBetweenChimneyAndRidgelineBis

    given Decoder[HorizontalDistanceBetweenChimneyAndRidgelineBis] =
        deriveDecoderForEnum[HorizontalDistanceBetweenChimneyAndRidgelineBis](
            HorizontalDistanceBetweenChimneyAndRidgelineBis.valueOf
        )
    given Encoder[HorizontalDistanceBetweenChimneyAndRidgelineBis] =
        deriveEncoderForEnum[HorizontalDistanceBetweenChimneyAndRidgelineBis]

    // HorizontalDistanceBetweenChimneyAndRidgeline

    given Decoder[HorizontalDistanceBetweenChimneyAndRidgeline] =
        deriveDecoderForEnum[HorizontalDistanceBetweenChimneyAndRidgeline](
            HorizontalDistanceBetweenChimneyAndRidgeline.valueOf
        )
    given Encoder[HorizontalDistanceBetweenChimneyAndRidgeline] =
        deriveEncoderForEnum[HorizontalDistanceBetweenChimneyAndRidgeline]

    // InnerConstructionMaterial

    given Decoder[InnerConstructionMaterial] =
        deriveDecoderForEnum[InnerConstructionMaterial](InnerConstructionMaterial.valueOf)
    ...
    given Decoder[OutsideAirIntakeAndChimneyLocations] =
        deriveDecoderForEnum[OutsideAirIntakeAndChimneyLocations](OutsideAirIntakeAndChimneyLocations.valueOf)
    ...
    given Encoder[OutsideAirLocationInHeater] =
        Encoder.instance { case x: OutsideAirLocationInHeater.FromBottom =>
            Encoder[OutsideAirLocationInHeater.FromBottom].apply(x)
        }

    // PipeLocation

    given Decoder[BoilerRoom]        = Decoder.decodeString.emap(s =>
        if (s == BoilerRoom.toString) Right(BoilerRoom) else Left(s"$s could not be decoded as 'BoilerRoom'")
    )
    given Decoder[HeatedArea]        = Decoder.decodeString.emap(s =>
        if (s == HeatedArea.toString) Right(HeatedArea) else Left(s"$s could not be decoded as 'HeatedArea'")
    )
    given Decoder[UnheatedInside]    = Decoder.decodeString.emap(s =>
        if (s == UnheatedInside.toString) Right(UnheatedInside)
        else Left(s"$s could not be decoded as 'UnheatedInside'")
    )
    given Decoder[OutsideOrExterior] = Decoder.decodeString.emap(s =>
        if (s == OutsideOrExterior.toString) Right(OutsideOrExterior)
        else Left(s"$s could not be decoded as 'OutsideOrExterior'")
    )
    ...
    given Decoder[PipeLocation.AreaHeatingStatus] =
        deriveDecoderForEnum[PipeLocation.AreaHeatingStatus](PipeLocation.AreaHeatingStatus.valueOf)
    ...
    given decoder_AreaName_BoilerRoom       : Decoder[AreaName.BoilerRoom]        = Decoder.decodeString.emap(s =>
        if (s == AreaName.BoilerRoom.toString) Right(AreaName.BoilerRoom)
        else Left(s"$s could not be decoded as 'AreaName.BoilerRoom'")
    )
    given decoder_AreaName_HeatedArea       : Decoder[AreaName.HeatedArea]        = Decoder.decodeString.emap(s =>
        if (s == AreaName.HeatedArea.toString) Right(AreaName.HeatedArea)
        else Left(s"$s could not be decoded as 'AreaName.HeatedArea'")
    )
    given decoder_AreaName_UnheatedInside   : Decoder[AreaName.UnheatedInside]    = Decoder.decodeString.emap(s =>
        if (s == AreaName.UnheatedInside.toString) Right(AreaName.UnheatedInside)
        else Left(s"$s could not be decoded as 'AreaName.UnheatedInside'")
    )
    given decoder_AreaName_OutsideOrExterior: Decoder[AreaName.OutsideOrExterior] = Decoder.decodeString.emap(s =>
        if (s == AreaName.OutsideOrExterior.toString) Right(AreaName.OutsideOrExterior)
        else Left(s"$s could not be decoded as 'AreaName.OutsideOrExterior'")
    )
    ...
    given encoder_AreaName_OutsideOrExterior: Encoder[AreaName.OutsideOrExterior] =
        Encoder.encodeString.contramap(_.toString)
    ...
    given Decoder[StandardOrComputationMethod] = Decoder.decodeString.emap { str =>
        StandardOrComputationMethod.values
            .find(_.reference == str)
    ...
    given Decoder[VerticalAngleBetweenChimneyAndAdjacentBuildings] =
        deriveDecoderForEnum[VerticalAngleBetweenChimneyAndAdjacentBuildings](
            VerticalAngleBetweenChimneyAndAdjacentBuildings.valueOf
        )
    given Encoder[VerticalAngleBetweenChimneyAndAdjacentBuildings] =
        deriveEncoderForEnum[VerticalAngleBetweenChimneyAndAdjacentBuildings]
