...
object PipeLocation:
    ...
    case object OutsideOrExterior
        extends PipeLocation(AreaName.OutsideOrExterior, AreaHeatingStatus.NotHeated)
        with Outside
    @Transl(I(_.pipe_location.outside_or_exterior))
    type OutsideOrExterior = OutsideOrExterior.type
    ...
    ) extends PipeLocation(area_name, area_heating_status)
        with InsideOrOutside derives Show:
