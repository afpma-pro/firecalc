...
trait Firebox_V3_Generators extends Firebox_V2_Generators:
    ...
    def genTestStandard: Gen[Firebox_V3.TestStandard] =
        Gen.frequency(
            (3, Gen.const(Firebox_V3.TestStandard.EN_15250)),
            (3, Gen.const(Firebox_V3.TestStandard.EN_13229)),
            (2, Gen.alphaStr.suchThat(_.nonEmpty).map(Firebox_V3.TestStandard.National(_)))
        )

    def genTraditional_V3: Gen[Firebox_V3.Traditional] =
        for
        ...
        yield Firebox_V3.Traditional(
            ...
            height_of_lowest_opening = height_of_first_row,
    ...
    def genAFPMA_PRSE_V3: Gen[Firebox_V3.AFPMA_PRSE] =
        for
            heat_output_reduced <- genHeatOutputReduced
    ...
    def genHeatOutputReducedTested: Gen[HeatOutputReduced.NotDefined_Or_Tested] =
        Gen.frequency(
            (1, Gen.const(HeatOutputReduced.NotDefined)),
            (3, Gen.choose(3.0, 20.0).map(v => HeatOutputReduced.FromTypeTest(v.kW)))
        )
    ...
    def genSingleTested_V3: Gen[Firebox_V3.SingleTested] =
        ...
        yield Firebox_V3.SingleTested(
            ...
            firebox_width                          = firebox_width,
            firebox_height                         = firebox_height,
            ash_pit_height                         = height_of_lowest_opening,
