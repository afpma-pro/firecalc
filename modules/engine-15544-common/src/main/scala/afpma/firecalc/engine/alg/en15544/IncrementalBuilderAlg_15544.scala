 ...
 *
 * Extracts duplicated logic from [[FlowOnlyIncrementalBuilder_15544]].
 * The builder mixes this trait in and provides its own [[PropsState]] type
 * and [[stateOps]] instance.
 *
 * EN 15544-specific differences from EN 13384:
 * - Split fallback: when no absolute direction is given, computes a
 *   perpendicular-to-Up vector (or Vec3.Rear if parallel) instead of
 *   falling back to the current frame direction.
 * - Merge fallback: when no absolute direction is given, applies the
 *   same perpendicular default and updates the frame/state accordingly,
 *   whereas EN 13384 leaves the frame untouched.
 *
 ...
 */
trait IncrementalBuilderAlg_15544[PS] {
     ...
     *
     * EN 15544-specific: when no absolute direction is provided, falls back to
     * a perpendicular-to-Up vector (or Vec3.Rear if vertical).
     */
     ...
     *
     * EN 15544-specific: when no absolute direction is provided, computes a
     * perpendicular default and updates the frame (unlike EN 13384 which
