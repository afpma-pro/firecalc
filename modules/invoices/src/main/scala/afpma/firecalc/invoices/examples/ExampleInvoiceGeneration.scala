...
import afpma.firecalc.invoices.InvoiceGenerator
...
import afpma.firecalc.invoices.templates.LogoPosition
import afpma.firecalc.invoices.templates.TemplateConfig as TemplateTemplateConfig
...
object ExampleInvoiceGeneration:
    ...
    def example1_DynamicInvoices(): Unit =
        ...
        InvoiceGenerator.generatePdf(
            ...
            client1Params,
            Locales.en,
            "modules/invoices/samples/simple_invoice.pdf"
        ) match
            case Right(file) =>
    ...
    def example2_CustomTemplate(): Unit =
        ...
        factory
            .generateInvoiceWithTemplate(invoiceParams, customTemplate, Locales.en)
    ...
    def example4_MultipleInvoices(): Unit =
        ...
        clients.foreach { case (clientName, invoiceNumber, amount) =>
            ...
            factory
                .generateInvoice(invoiceParams, Locales.en)
                .map: invoiceResult =>
                    invoiceResult.savePdfToFile(
                        s"modules/invoices/samples/invoice_${clientName.replace(" ", "_").toLowerCase}.pdf"
                    ) match
                        case Right(file) =>
                        ...
                        case Left(error) =>
                            ...
                            invoiceResult
                .map: invoiceResult =>
                    invoiceResult.saveTypToFile(
                        s"modules/invoices/samples/invoice_${clientName.replace(" ", "_").toLowerCase}.typ"
                    ) match
                        case Right(file) =>
                            println(s"✅ Invoice for $clientName: ${file.getName}")
                        case Left(error) =>
    ...
    def example5_TaxExemptInvoice(): Unit =
        ...
        val factory = FireCalcInvoiceFactory.fromTemplate(companyConfig)
        factory
            .generateInvoice(taxExemptInvoiceParams, Locales.fr) // Generate in French for this example
    ...
    def createSampleInvoice(): InvoiceData =
        ...
        InvoiceData     (
            ...
            currency           = "EUR",
            notes              = Some(
                "Thank you for your business! For any questions regarding this invoice, please contact our accounting department."
            ),
            discountPercentage = Some(BigDecimal("0")),
            status             = InvoiceStatus.Draft
        )
