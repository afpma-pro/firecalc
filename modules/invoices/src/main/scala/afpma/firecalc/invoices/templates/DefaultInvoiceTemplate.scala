...
import afpma.firecalc.invoices.typst.MoneyFormatting
import afpma.firecalc.invoices.typst.TypstElements
import afpma.firecalc.invoices.typst.TypstShow
 ...
 */
class DefaultInvoiceTemplate(val config: TemplateConfig = TemplateConfig()) extends ConfigurableTemplate[InvoiceData]:
    ...
    private def invoiceHeader(invoice: InvoiceData)(using i18n: I18nData_Invoices): String =
        val logoSection =
            if (invoice.sender.logo.isDefined)
    ...
    private def totalsSection(invoice: InvoiceData)(using i18n: I18nData_Invoices): String =
        val subtotalRow =
            s"[*${i18n.totals.subtotal}*], [${MoneyFormatting.formatCurrency(invoice.subtotal, invoice.currency)}],"
        ...
        val totalRow =
            s"[*${i18n.totals.total}*], [*${MoneyFormatting.formatCurrency(invoice.totalAmount, invoice.currency)}*],"
