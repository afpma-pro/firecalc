...
object TypstShow:
    ...
    def mkForTable[A](
        ...
        TypstShow.makeRaw { a =>
            ...
            val alignments =
                if (columnAlignment.length >= colCount) columnAlignment
                else columnAlignment ++ Seq.fill(colCount - columnAlignment.length)("left")
...
extension [A: TypstShow](a: A) def toTypst: TypstShow.SanitizedString = TypstShow[A].showAsTypst(a)
...
object InvoiceTypstInstances:
    ...
    given (using i18n: I18nData_Invoices): TypstShow[Company] = TypstShow.makeRaw { company =>
        ...
        val reg_number_line =
            company.registrationNumber.map(reg => s"${i18n.company.registration_label} $reg".sanitized)
    ...
    given (using i18n: I18nData_Invoices): TypstShow[PaymentMethod] = TypstShow.make { method =>
        method match
            case PaymentMethod.BankTransfer(iban, bic) =>
                val details = Seq(
                    iban.map(i => s"${i18n.payment.iban_label} $i"),
                    bic.map (b => s"${i18n.payment.bic_label} $b" )
                ).flatten
                if (details.nonEmpty) s"${i18n.payment.bank_transfer} (${details.mkString(", ")})"
                else i18n.payment.bank_transfer
...
object TypstElements:
    ...
    def block(width: String = "100%", height: Option[String] = None, fill: Option[String] = None)(
        content: String
    ): String =
    ...
    def text(size: Option[String] = None, weight: Option[String] = None, style: Option[String] = None)(
        content: String
    ): String =
        val attrs = Seq(
            size.map  (s => s"size: $s"       ),
