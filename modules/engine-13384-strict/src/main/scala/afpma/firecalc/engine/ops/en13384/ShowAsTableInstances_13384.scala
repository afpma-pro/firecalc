...
class ShowAsTableInstances_13384(using locale: Locale):
    ...
    given showAsTable_ReferenceTemperatures: ShowAsTable[ReferenceTemperatures] =
        ShowAsTable.mkLightFor(
            ...
            x =>
                ...
                val out =
                    (s"${I18N.en13384.exterior_air_temperature} (T_L)"                         :: s"${I18N.draft_min}" :: tl_map
                        .get(DraftCondition.draftMin)
                        .get
                        .to_degC
                        .show                                                                  :: Nil) ::
                        (s"${I18N.en13384.exterior_air_temperature} (T_L)"                     :: s"${I18N.draft_max}" :: tl_map
                            .get(DraftCondition.draftMax)
                            .get
                            .to_degC
                            .show                                                              :: Nil) ::
                ...
                pressures.underPressure match
                    case UnderPressure.Negative =>
                        (_I.pressures.underPressure          :: ""      :: _I.pressures.underPressure_negative :: Nil) ::
                            (_I.pressures.flue_gas_draft_min :: "PW"    :: pressures.flue_gas_draft_min.showOrElse(
                                "-"
                            )                                :: Nil) ::
                            (_I.pressures.flue_gas_draft_max :: "PWmax" :: pressures.flue_gas_draft_max.showOrElse(
                                "-"
                            )                                :: Nil) ::
                            Nil
                    case UnderPressure.Positive =>
                        (_I.pressures.underPressure          :: ""       :: _I.pressures.underPressure_positive :: Nil) ::
                            (_I.pressures.flue_gas_pdiff_min :: "PWOmin" :: pressures.flue_gas_pdiff_min.showOrElse(
                                "-"
                            )                                :: Nil) ::
                            (_I.pressures.flue_gas_pdiff_max :: "PWO"    :: pressures.flue_gas_pdiff_max.showOrElse(
                                "-"
                            )                                :: Nil) ::
                            Nil
            val volumeFlowsLines =
                volumeFlows.fold(Nil): vflows =>
                    (_I.volumeFlows.flue_gas_volume_flow_nominal           :: "" :: vflows.flue_gas_volume_flow_nominal.to_m3_per_h.show       :: Nil) ::
                        (_I.volumeFlows.flue_gas_volume_flow_reduced       :: "" :: vflows.flue_gas_volume_flow_reduced
                            .map(_.to_m3_per_h)
                            .showOrElse("-")                               :: Nil) ::
                        (_I.volumeFlows.combustion_air_volume_flow_nominal :: "" :: vflows.combustion_air_volume_flow_nominal.to_m3_per_h.show :: Nil) ::
                        (_I.volumeFlows.combustion_air_volume_flow_reduced :: "" :: vflows.combustion_air_volume_flow_reduced
                            .map(_.to_m3_per_h)
                            .showOrElse("-")                               :: Nil) ::
                        Nil
            val out =
                (I18N.firebox.ref                                  :: ""       :: reference.show                             :: Nil) ::
                    ...
                    (_I.fluegas.co2_dry_perc_reduced               :: "σ_CO2"  :: fluegas.co2_dry_perc_reduced.showOrElse(
                        "-"
                    )                                              :: Nil) ::
                    ...
                    (_I.temperatures.flue_gas_temp_reduced         :: "TW min" :: temperatures.flue_gas_temp_reduced.showOrElse(
                        "-"
                    )                                              :: Nil) ::
                    (_I.massFlows.flue_gas_mass_flow_nominal       :: "ṁ"      :: massFlows.flue_gas_mass_flow_nominal.showOrElse(
                        "-"
                    )                                              :: Nil) ::
                    (_I.massFlows.flue_gas_mass_flow_reduced       :: "ṁ min"  :: massFlows.flue_gas_mass_flow_reduced
                        .showOrElse("-")                           :: Nil) ::
                    (_I.massFlows.combustion_air_mass_flow_nominal :: "ṁB"     :: massFlows.combustion_air_mass_flow_nominal
                        .showOrElse("-")                           :: Nil) ::
                    (_I.massFlows.combustion_air_mass_flow_reduced :: "ṁB min" :: massFlows.combustion_air_mass_flow_reduced
                        .showOrElse("-")                           :: Nil) ::
                    volumeFlowsLines                              :::
                    pressuresLines                                :::
                    Nil
    ...
    given showAsTable_NationalAcceptedData: ShowAsTable[NationalAcceptedData] =
        ShowAsTable.mkLightFor(
            ...
            "symbol" :: "conditions" :: "value" :: Nil,
            nad =>
                val t_L_override_at_draftMin = nad.T_L_override
                    .map(_.get(DraftCondition.DraftMinOrPositivePressureMax).map(_.to_degC.show).getOrElse("-"))
                    .getOrElse("-")
                val t_L_override_at_draftMax = nad.T_L_override
                    .map(_.get(DraftCondition.DraftMaxOrPositivePressureMin).map(_.to_degC.show).getOrElse("-"))
                    .getOrElse("-")
