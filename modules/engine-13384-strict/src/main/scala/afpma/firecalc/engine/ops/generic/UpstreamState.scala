...
object UpstreamState:
     ...
     *
     * When `ComputeAt.Mean` is requested but the pipe result lacks mean values
     * (e.g. EN 15544 flow-only sections don't compute `v_mean`), falls back to
     * the middle-position values. This ensures that a direct flue→chimney
     * topology (no connector) still propagates density and velocity correctly.
     */
