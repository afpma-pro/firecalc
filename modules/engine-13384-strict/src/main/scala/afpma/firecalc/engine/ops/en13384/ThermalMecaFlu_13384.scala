...
import afpma.firecalc.engine.standard.{EN13384_FormulaError, MecaFlu_Error, SlotContext}
...
object ThermalMecaFlu_13384 extends MecaFlu_13384_Alg with HasTypeMembers_13384_WithThermalAirIntake:
    ...
    override type PipeDescr = ThermalPipeDescr_13384.type
    override val pipeDescr: PipeDescr = ThermalPipeDescr_13384
    ...
    def makePipeSectionResult(
        gp                   : GasInPipeEl[NamedPipeElDescrG[PipeElDescr], Gas, Params],
...
private abstract trait MecaFlu_EN13384_PipeSectionResult_Impl(
    ...
    private def T_u(using asd: AirSpaceDetailed): Either[EN13384_FormulaError.NoOutsideSurface, TempD[Kelvin]] =
        ...
        val t_u_custom_area = custAreaAmbAirTempSet map en13384.T_u_temp_helper(
            T_L,
            airSpace,
            pReq,
    ...
    private def mkPipeWithGasFlowWithLength(
        ...
        models.en13384.ThermalPipeDescr_13384.Error.OnlyAStraightSectionCanBeConvertedToPipeWithGasFlow.type,
        PipeWithGasFlow
    ] =
        ...
        MecaFluOps.whenGasType(gp.pipeEl.typ)(
            ...
            ifFlueGas       = curr.el match
                case s: StraightSection =>
                    en13384.formulas
                        .thermal_resistance_for_layers_calc(t_emitting_layer, innerShape(using Start), s.layers)
                        .leftMap(formulaError =>
                            MecaFlu_Error.ThermalResistanceCalculationErrors(
    ...
    val en13384_tr = curr.el match
        case _: StraightSection =>
            t_i_middle_b_approx(0.m2_K_per_W) // take middle instead of mean (as approximation)
                .flatMap(compute_thermal_resistance)
                .toOption
        case _ => None
                ...
                prevO match
                    ...
                    case Some(prev) =>
                        val prev_density_mean = en13384_density_mean(
                            prev.gas_temp_mean.getOrElse(
                                throw new Exception(
                                    s"could not compute 'gas_temp_mean' on previous section ${prev.section_full_descr}}"
                                )
                            ),
                            prev.section_typ,
                            pReq
                        ...
                        en13384.P_G_calc(
                            prev_density_mean,
                            prev.v_mean.getOrElse(
                                throw new Exception(
                                    s"could not compute 'v_mean' on previous section ${prev.section_full_descr}}"
                                )
                            ),
                            d_mean,
    ...
    private def temperature_i_x_b_calc(
        ...
        val debug =
            if (
                curr.fullRef.contains(
                    "ChimneyPipeT #2 ='partie en extérieur'"
                ) && (LoadQty.summon == LoadQty.Nominal) && false
            ) true
            else false
        if     (debug) println("-///////////////////////////////////////////////////////////")
        curr.typ match
            case ConnectorPipeT | ChimneyPipeT =>
                mkPipeWithGasFlowWithLength(
                    temp_mean,
                    massFlow,
                    exteriorAir,
                    custom_section_length
                ) match // te, temp_mean, to ???
                    ...
                    case Left(
                            models.en13384.ThermalPipeDescr_13384.Error.OnlyAStraightSectionCanBeConvertedToPipeWithGasFlow
                        ) =>
                        prevO match
            ...
            case pt                            =>
                MecaFlu_Error
                    .UnexpectedPipeType(s"Tiob can only be computed for connector pipe or chimney pipe, not '$pt'", pt)
                    .asLeft
...
private abstract trait MecaFlu_13384_PipeResult_Impl(
    ...
    with ThermalMecaFlu_Helpers:
    import MecaFlu_13384_PipeResult_Impl.MapState
    ...
    private val _out =
        ...
        val (_, results) = fd.elements.mapAccumulate(initS) { (st, elem) =>
            ...
            val psr = ThermalMecaFlu_13384.makePipeSectionResult(
                gp,
                hafg,
                hamf,
                st.gas_temp_start,
...
object MecaFlu_13384_PipeResult_Impl:
