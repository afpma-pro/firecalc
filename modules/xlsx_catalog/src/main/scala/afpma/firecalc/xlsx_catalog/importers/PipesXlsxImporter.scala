...
object PipesXlsxImporter:
    ...
    private def parseRow(
        ...
        image: Option[String]
    ): Option[SetThermalPipeProp_13384.SetPropertiesInBatch] =
        readString(row, PipeCols.BatchName).map: batchName =>
            ...
            if layers.nonEmpty then props += SetThermalPipeProp_13384.SetLayers(layers.toList)
    ...
    private[importers] def parseMaterial(name: String, roughnessOverride: Option[Double]): Material_13384 =
        ...
        name match
            ...
            case "CorrugatedMetal" =>
                rough.fold(Material_13384.CorrugatedMetal())(r => Material_13384.CorrugatedMetal(r))
