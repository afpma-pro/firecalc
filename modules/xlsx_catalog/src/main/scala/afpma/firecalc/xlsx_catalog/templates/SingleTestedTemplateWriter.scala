...
object SingleTestedTemplateWriter:
    ...
    private def buildMainSheet(sheet: Sheet, styles: Styles.StyleBundle, data: Option[Firebox.SingleTested]): Unit =
        ...
        writeFormField(
            ...
            "Ash pit height",
            "m",
            v(_.ash_pit_height.value, 0.05)
        )

        // ── Glass ──
        writeSection(sheet, styles, SectionGlass, "Vitrage / Glazing", 4)
        val glassStr: Firebox.SingleTested => String = fb =>
            if fb.is_glass_surface_ratio_below_one_fifth then "Yes" else "No"
        writeFormField(
            sheet,
            styles,
            GlassRatioBelow,
            "Ratio surface vitrée < 1/5",
            "Glass surface ratio < 1/5",
            "—",
            v(glassStr, "Yes")
        )
        ...
        writeFormField(
            ...
            styles,
            MeanFireboxTemp,
            "Température moyenne du foyer",
            "Mean firebox temperature",
            "°C",
            vOpt(_.mean_firebox_temperature.map(_.value)),
            optional = true
        )
        writeFormField(
            sheet,
        ...
        writeFormField(
            ...
            "Nominal efficiency",
            "%",
            v(_.efficiency_nominal.value, 75.0)
        )
        writeFormField(
            sheet,
            styles,
            ...
            "Reduced efficiency",
            "%",
            vOpt(_.efficiency_reduced.map(_.value)),
            optional = true
        )
        ...
        writeFormField(
            ...
            "Min. fuel mass",
            "kg",
            vOpt(_.minimum_fuel_mass.map(_.value)),
            optional = true
        )
        writeFormField(
            sheet,
            ...
            "Max. fuel mass",
            "kg",
            v(_.maximum_fuel_mass.value, 10.0)
        )
        ...
        writeFormField(
            ...
            styles,
            AirFuelNominal,
            "Ratio air/combustible nominal",
            "Air-fuel ratio nominal",
            "—",
        ...
        writeFormField(
            ...
            styles,
            AirFuelLowest,
            "Ratio air/combustible minimal",
            "Air-fuel ratio lowest",
            "—",
            vOpt(_.air_fuel_ratio_lowest.map(_.value)),
            optional = true
        )
        ...
        writeFormField(
            ...
            "Nominal dry CO2",
            "%",
            v(_.co2_dry_nominal.value, 12.0)
        )
        writeFormField(
            sheet,
            styles,
            ...
            "Lowest dry CO2",
            "%",
            vOpt(_.co2_dry_lowest.map(_.value)),
            optional = true
        )
        ...
        writeFormField(
            ...
            styles,
            PelletsBurnDur,
            "Durée de combustion de la charge",
            "Pellets load burn duration",
            "min",
            vOpt(_.pellets_load_burn_duration.map(_.value)),
            optional = true
        )
        addComment    (sheet, PelletsBurnDur, 0, "Uniquement pour les appareils à granulés.\n\nOnly for pellet appliances.")
