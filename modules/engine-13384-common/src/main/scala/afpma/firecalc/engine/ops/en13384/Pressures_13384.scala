...
trait Pressures_13384:
    ...
    extension (a: A)
        ...
        final def `en13384_pr_all-ph`: ValidatedNel[MecaFlu_Error, Pressure] =
            (en13384_pr_all, en13384_ph).mapN: (pr_all, ph) =>
                pr_all - ph

object Pressures_13384:

    type ForPipeResult = Pressures_13384 {
        type A = PipeResult
    }

    given ForPipeResult = new Pressures_13384 {
        type A = PipeResult
        extension (a: A)
            def en13384_ph: ValidatedNel[MecaFlu_Error, Pressure] =
                a.ph.validNel
