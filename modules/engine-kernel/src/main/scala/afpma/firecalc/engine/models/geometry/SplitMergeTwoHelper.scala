...
case class SplitMergeTwoHelper private (
    ...
    /** Start position of branch two: displaced from splitPosition along reflectedBranchDirection. */
    val branchTwoStartPosition: Vec3 = splitPosition + (reflectedBranchDirection * branchOneOffset)
...
object SplitMergeTwoHelper:
    ...
    /**
     * Construct a `SplitMergeTwoHelper` from the given directions, position, and offset.
     *
     ...
     * @param splitPosition 3D anchor position of the split plane Π
     * @param branchOneOffset non-negative distance from `splitPosition` to each branch start
    ...
    def safe(
        ...
        else Right(SplitMergeTwoHelper(iNorm, o1Norm, splitPosition, branchOneOffset))

    /**
     * Compute the expected merge position for merge-position validation.
     *
     * The merge plane is the plane perpendicular to the incoming direction
     * passing through the split position. The expected merge position is the
     * orthogonal projection of the branch tip onto this plane.
     *
     * This differs from `computeMergePosition` on the helper instance, which
     * projects onto the split plane Π spanned by {incoming, branchOne} — that
     * plane is used for auto-merge geometry, not validation.
     *
     ...
     * @return expected merge position (projection of branch tip onto the incoming-perpendicular plane)
     */
    def computeExpectedMergePosition(
        ...
        val distAlong = offset.dot(incomingDirection)
        branchTipPosition - incomingDirection * distAlong
