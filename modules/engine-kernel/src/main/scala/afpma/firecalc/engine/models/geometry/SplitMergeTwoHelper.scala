...
object SplitMergeTwoHelper:
    ...
    def computeExpectedMergePosition(
        ...
        println(f"[DEBUG computeExpectedMergePosition]"                                                                )
        println(
            f"  incomingDirection    = (${incomingDirection.x}%,.6f, ${incomingDirection.y}%,.6f, ${incomingDirection.z}%,.6f)"
        )
        ...
        println(f"  symmetryPlaneConfig  = ${symmetryPlaneConfig.toString()}"                                          )
        val symmetryPlaneNormal = SymmetryPlaneConfig.normal(incomingDirection, symmetryPlaneConfig)
        println(
            f"  symmetryPlaneNormal  = (${symmetryPlaneNormal.x}%,.6f, ${symmetryPlaneNormal.y}%,.6f, ${symmetryPlaneNormal.z}%,.6f)"
        )
        val offset = branchTipPosition - splitPosition
        println(f"  offset (tip-split) = (${offset.x}%,.6f, ${offset.y}%,.6f, ${offset.z}%,.6f)")
        val distAlong = offset.dot(symmetryPlaneNormal)
        println(f"  distAlong          = ${distAlong}%,.6f")
        val result = branchTipPosition - symmetryPlaneNormal * distAlong
        println(f"  expectedMergePos   = (${result.x}%,.6f, ${result.y}%,.6f, ${result.z}%,.6f)")
        println(f"[END DEBUG computeExpectedMergePosition]"                                     )
        result
