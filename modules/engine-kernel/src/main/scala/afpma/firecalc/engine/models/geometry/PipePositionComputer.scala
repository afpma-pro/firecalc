...
object PipePositionComputer:
    ...
    private def rayIntersectBoxSurface(dir: Vec3, halfWidth: Double, halfDepth: Double): (Double, Double) =
        ...
        else (dx * t + FireboxCoordinateSystem.FireboxBaseCenterX, dy * t + FireboxCoordinateSystem.FireboxBaseCenterY)
    ...
    private def computeSideStart(
        ...
        val (x, y)    = (projectOnBoundary(dir, halfWidth, halfDepth) match {
            case (px, py) =>
                (px + FireboxCoordinateSystem.FireboxBaseCenterX, py + FireboxCoordinateSystem.FireboxBaseCenterY)
        })
