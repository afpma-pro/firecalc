...
object Preview:
    ...
    def make_summary_line(
        ...
        summary_values : List[String],
        descr_idx      : Option[Int] = None,
        descr_value    : String      = ""
    )(using Locale): List[String] =
        val summary_indices = _find_indices_for(summary_headers).getOrElse(
            throw new IllegalStateException(s"make sur headers contains '${summary_headers.mkString(", ")}'")
        )
        val summary         = (summary_indices
            .zip(summary_values))
            .foldLeft(empty_line):
