...
import afpma.firecalc.units.coulombutils.*
import afpma.firecalc.units.coulombutils.show_Angle
import afpma.firecalc.units.coulombutils.show_Meters
import afpma.firecalc.units.coulombutils.show_Pascals
import afpma.firecalc.units.coulombutils.show_SquareMeters
...
object FlowOnlyPipeDescr_13384 extends afpma.firecalc.engine.models.PipeDescrAlg:
    ctx =>
    ...
    object DirectionChange:
        given Show[
            AngleVifDe0A90_Unsafe | AngleVifDe0A90 | CoudeCourbe90 | CoudeCourbe90_Unsafe | CoudeCourbe60 |
                CoudeCourbe60_Unsafe
        ] =
            Show.show:
    ...
    case class AngleSpecifique(
        ɣ                          : QtyD[Degree],
        zeta                       : ζ,
        override val angleN2       : Option[QtyD[Degree]] = None,
        override val effectiveShape: PipeShape
    ) extends DirectionChange(ɣ, angleN2, effectiveShape) derives Show

    case class AngleVifDe0A90(
        ɣ                          : QtyD[Degree],
        Ld                         : QtyD[Meter],
        Dh                         : QtyD[Meter],
        override val angleN2       : Option[QtyD[Degree]] = None,
    ...
    // __INTERPRETATION__
    case class AngleVifDe0A90_Unsafe(
        ɣ                          : QtyD[Degree],
        Ld                         : QtyD[Meter],
        Dh                         : QtyD[Meter],
        override val angleN2       : Option[QtyD[Degree]] = None,
    ...
    case class CoudeCourbe90(
        R                          : QtyD[Meter],
        Dh                         : QtyD[Meter],
        Ld                         : QtyD[Meter],
        override val angleN2       : Option[QtyD[Degree]] = None,
        override val effectiveShape: PipeShape
    ) extends DirectionChange(90.degrees, angleN2, effectiveShape) derives Show

    // __INTERPRETATION__
    case class CoudeCourbe90_Unsafe(
        R                          : QtyD[Meter],
        Dh                         : QtyD[Meter],
        Ld                         : QtyD[Meter],
        override val angleN2       : Option[QtyD[Degree]] = None,
    ...
    case class CoudeCourbe60(
        R                          : QtyD[Meter],
        Dh                         : QtyD[Meter],
        Ld                         : QtyD[Meter],
        override val angleN2       : Option[QtyD[Degree]] = None,
        override val effectiveShape: PipeShape
    ) extends DirectionChange(60.degrees, angleN2, effectiveShape) derives Show

    // __INTERPRETATION__
    case class CoudeCourbe60_Unsafe(
        R                          : QtyD[Meter],
        Dh                         : QtyD[Meter],
        Ld                         : QtyD[Meter],
        override val angleN2       : Option[QtyD[Degree]] = None,
    ...
    case class CoudeASegment90Avec2A45(
        ...
        override val angleN2       : Option[QtyD[Degree]] = None,
        override val effectiveShape: PipeShape
    ) extends CoudeASegment90(2, R, Dh, angleN2, effectiveShape) derives Show
    case class CoudeASegment90Avec3A30(
        override val R             : QtyD[Meter],
        override val Dh            : QtyD[Meter],
        override val angleN2       : Option[QtyD[Degree]] = None,
        override val effectiveShape: PipeShape
    ) extends CoudeASegment90(3, R, Dh, angleN2, effectiveShape) derives Show
    case class CoudeASegment90Avec4A22p5(
        override val R             : QtyD[Meter],
        override val Dh            : QtyD[Meter],
        override val angleN2       : Option[QtyD[Degree]] = None,
        override val effectiveShape: PipeShape
    ) extends CoudeASegment90(4, R, Dh, angleN2, effectiveShape) derives Show
    ...
    case class SectionDecrease(override val from: PipeShape.Circle, override val to: PipeShape.Circle)
        extends SectionGeometryChange(from, to) derives Show
