...
object AirIntakeReplay:
    /** Result of a single auto-mode computation: effective start position and box connection point. */
    case class AirIntakeAutoResult(startPosition: Position3D, connectionPoint: Position3D)
    ...
    /**
     * Single source of truth: replay once, compute both start position and connection point.
     *
     ...
     *   3. displacement = target - rawFinal
     *   4. Return both start position (effective start) and connectionPoint
     */
    def computeAirIntakeAuto(
        ...
        if descr.isEmpty then
            ...
            // offset is a displacement; add to replay origin to get absolute start position
            val origin      = Vec3(
                FireboxCoordinateSystem.FireboxBaseCenterX,
                FireboxCoordinateSystem.FireboxBaseCenterY,
                FireboxCoordinateSystem.FireboxBaseCenterZ
            )
            val startPos    = origin + offset
            val startPosPos = Position3D(startPos.x.m, startPos.y.m, startPos.z.m)

            AirIntakeAutoResult(startPosPos, connectionPoint)

    /**
     * Compute the effective start position for an air intake pipe in Auto mode.
     * Thin wrapper around [[computeAirIntakeAuto]] returning the start position.
     */
