...
private object cli_table:
    ...
    class CliTable(val separators: TableSeparator):
            ...
            case _     =>
                val sizes           =
                    for (row <- table)
                        yield
                            for (cell <- row)
                                yield if cell == null then 0 else cell.toString.length
        ...
        private def formatRow(row: Seq[Any], colSizes: Seq[Int]): String =
            val cells =
                for (item, size) <- row zip colSizes yield if size == 0 then "" else alignLeft(item.toString, size)
