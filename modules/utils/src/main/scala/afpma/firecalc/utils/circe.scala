...
object circe:
    ...
    given deriveEncoderForEither: [A, B] => (ea: Encoder[A]) => (eb: Encoder[B]) => Encoder[Either[A, B]] =
    ...
    given deriveDecoderForEither: [A, B] => (da: Decoder[A]) => (db: Decoder[B]) => Decoder[Either[A, B]] =
                            ...
                            lr match
                                ...
                                case _       =>
                                    Left(
                                        DecodingFailure(
                                            Reason.CustomReason(
                                                s"expected 'type' to equal 'either' and 'left_or_right' to equal 'left' or 'right' but got 'type'=$tpe and 'left_or_right'=${lr}"
                                            ),
                                            lrCursor
                                        )
                                    )
    ...
    given deriveEncoderForOptionOfEither
        : [A, B] => (ea: Encoder[A]) => (eb: Encoder[B]) => Encoder[OptionOfEither[A, B]] =
        new Encoder[OptionOfEither[A, B]]:
            def apply(a: OptionOfEither[A, B]): Json = Json.obj(
                ("type", Json.fromString("option_of_either")),
                (
                    "none_or_left_or_right",
                    a match
    ...
    given deriveDecoderForOptionOfEither
        : [A, B] => (da: Decoder[A]) => (db: Decoder[B]) => Decoder[OptionOfEither[A, B]] =
        new Decoder[OptionOfEither[A, B]]:
                    ...
                    ret <- {
                        if (tpe == "option_of_either")
                            nlr match
                                ...
                                case _       =>
                                    Left(
                                        DecodingFailure(
                                            Reason.CustomReason(
                                                s"expected 'type' to equal 'option_of_either' and 'none_or_left_or_right' to equal 'none', 'left' or 'right' but got 'type'=$tpe and 'none_or_left_or_right'=${nlr}"
                                            ),
                                            lrCursor
                                        )
                                    )
                            ...
                            Left(
                                DecodingFailure(
                                    Reason.CustomReason(
                                        s"expected 'type' to equal 'option_of_either' but got 'type'= '$tpe'"
                                    ),
                                    lrCursor
                                )
                            )
