...
class EN15544_Labo_Formulas(
    ...
    extends EN15544_MCE_Formulas(net_calorific_value_of_wet_wood, net_calorific_value_of_dry_wood):
...
object EN15544_Labo_Application:
    ...
    trait SpecificPipeLocations:
        ...
        object Area:
            val EnterreExterieur   = CustomArea.sameTuForAllConditions(
                "EnterreExterieur",
                NotHeated,
                inside = false,
                ambiant_air_temperatures.EnterreExterieur
            )
            val EnterreInterieur   = CustomArea.sameTuForAllConditions(
                "EnterreInterieur",
                NotHeated,
                inside = true,
                ambiant_air_temperatures.EnterreInterieur
            )
            val AirDansLePoele     = CustomArea.sameTuForAllConditions(
                "AirDansLePoele",
                Heated,
                inside = true,
                ambiant_air_temperatures.AirDansLePoele
            )
            val Foyer              =
                CustomArea.sameTuForAllConditions("Foyer", Heated, inside = true, ambiant_air_temperatures.Foyer)
            val Accumulateur       = CustomArea.sameTuForAllConditions(
                "Accumulateur",
                Heated,
                inside = true,
                ambiant_air_temperatures.Accumulateur
            )
            val DansLaPieceDuPoele = CustomArea.sameTuForAllConditions(
                "DansLaPieceDuPoele",
                Heated,
                inside = true,
                ambiant_air_temperatures.DansLaPieceDuPoele
            )
            val Chauffee           =
                CustomArea.sameTuForAllConditions("Chauffee", Heated, inside = true, ambiant_air_temperatures.Chauffee)
            val NonChauffee        = CustomArea.sameTuForAllConditions(
                "NonChauffee",
                NotHeated,
                inside = true,
                ambiant_air_temperatures.NonChauffee
            )
            val Exterieure         = CustomArea.sameTuForAllConditions(
                "Exterieure",
                NotHeated,
                inside = false,
                ambiant_air_temperatures.Exterieure
            )
