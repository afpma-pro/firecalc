...
object v1:
    ...
    object CustomerType:
        ...
        given Decoder[CustomerType] =
            Decoder[String].emap(s => CustomerType.values.find(_.toString == s).toRight(s"Invalid customer type: $s"))
    ...
    object ProductMetadata:
        ...
        given Encoder[ProductMetadata]            =
            Encoder.instance { case x: FileDescriptionWithContent =>
                Encoder[FileDescriptionWithContent].apply(x)
            }
        ...
        extension (fdc: FileDescriptionWithContent)
            def toFile: Either[String, java.io.File] =
                try
                    ...
                    val decodedBytes =
                        try Base64.getDecoder.decode(fdc.content)
                        catch
                    ...
                    // Create temp file
                    val tempFile =
                        try java.io.File.createTempFile("firecalc_", s"_${fdc.filename}")
                        catch
