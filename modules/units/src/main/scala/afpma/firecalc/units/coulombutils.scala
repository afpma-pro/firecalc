...
object coulombutils:
    ...
    export coulomb.units.mks.{Watt, ctx_unit_Watt, Joule, ctx_unit_Joule, Radian, ctx_unit_Radian}

    export coulomb.units.mksa.{Pascal, ctx_unit_Pascal}

    export coulomb.units.us.{Inch, ctx_unit_Inch, Foot, ctx_unit_Foot, Pound, ctx_unit_Pound, BTU, ctx_unit_BTU}
    ...
    export coulomb.units.time.{Hour, ctx_unit_Hour, Minute, ctx_unit_Minute}

    export coulomb.units.temperature.{Celsius, ctx_unit_Celsius, Fahrenheit, ctx_unit_Fahrenheit, withTemperature}
    ...
    object conversions:
        ...
        extension (x: Area         ) def to_cm2: QtyD[(Centimeter ^ 2)] = x.toUnit[(Centimeter ^ 2)]
        extension (x: Dimensionless) def toPercent = (x.value * 100.0).withUnit[Percent]
        ...
        extension (x: Percentage) def asRatio    : QtyD[1]                  = x.toUnit[1]
        extension (x: VolumeFlow) def to_m3_per_h: QtyD[(Meter ^ 3) / Hour] = x.toUnit[(Meter ^ 3) / Hour]
        extension (x: TKelvin   ) def to_degC    : TCelsius                 = x.toUnit[Celsius]
        extension (c: TCelsius  ) def to_degK    : TKelvin                  = c.toUnit[Kelvin]
    ...
    extension [A: Show](a: A      ) def showP: String = Show[A].show(a)
    ...
    object shows:
        ...
        def mkShowPrettyForQtyD_RoundedUpNearZero[U, PrettyU: ShowUnit](
            ...
            Show.show: qu =>
                require(
                    prettyFmt.startsWith("%.") && prettyFmt.endsWith("f"),
                    s"[ERR]: unexpected prettyFmt ${prettyFmt}"
                )
