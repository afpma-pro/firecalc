...
object PipeChainGeneric:
    ...
    def build(slots: Seq[PostFireboxPipeDescrSlot]): Vector[SlotBuildResult] =
        slots.foldLeft((Vector.empty[SlotBuildResult], Option.empty[PipeFrame])):
    ...
    private def buildSlot(
        ...
        slot match
            case FlueSlot(descr)        => buildFlue15544(descr)
    ...
    // ── FlowOnly 15544 flue (no external frame — it's always first) ─────

    private def buildFlue15544(
        descr: Seq[FluePipe_Module_15544.incremental.IncrDescr]
    ): SlotBuildResult =
        val (fullDescrResult, finalFrameV) =
            FluePipe_Module_15544.mkPipeFromIncrDescrWithFinalFrame(descr)
