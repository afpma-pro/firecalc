...
import coulomb.ops.algebra.all.* // scalafix:ok
...
class ShowAsTableInstances_15544(using Locale):
    ...
    given showAsTable_PressureRequirement_EN15544: ShowAsTable[PressureRequirement] =
        ...
        ShowAsTable.mkLightFor(I18N.headers.pressure_requirements_15544) { pcond =>
            ...
            (s"${_I.sum_of_all_resistances} (Σ pr + Σ pu)"                                          :: (sum_pr_pu).show :: "" :: Nil) ::
                ...
                (s"${_I.pressure_difference} (Σ ph - (Σ pr + Σ pu) >= ${P0.show})"                  :: (sum_ph - sum_pr_pu).show :: status_ok_or_not(
                    sum_ph - sum_pr_pu >= P0
                )                                                                                   :: Nil) ::
                (s"${_I.pressure_difference} (Σ ph - (Σ pr + Σ pu) <= ${(0.05 * sum_pr_pu).showP})" :: (sum_ph - sum_pr_pu).show :: status_ok_or_not(
                    sum_ph - sum_pr_pu <= 0.05 * sum_pr_pu
                )                                                                                   :: Nil) ::
                Nil
        }
    ...
    given showAsTable_Inputs: ShowAsTable[Inputs_15544_Alg] =
        ShowAsTable.mkLightFor(
            I18N.inputs_data,
            "term" :: "value" :: Nil,
            i =>
