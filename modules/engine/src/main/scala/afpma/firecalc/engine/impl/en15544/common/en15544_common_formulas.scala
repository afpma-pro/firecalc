...
trait EN15544_V_2023_Common_Formulas extends EN15544_V_2023_Formulas_Alg:
    ...
    override lazy val V_L_calc =
        (mb, ft, fs) => (0.00256 * mb.value * ft.value * fs.value).withUnit[(Meter ^ 3) / Second]
    ...
    override lazy val V_G_calc =
        (mb, ft, fs) => (0.00273 * mb.value * ft.value * fs.value).withUnit[(Meter ^ 3) / Second]
    ...
    override lazy val ρ_L_calc =
        (ft, fs) => (1.293 / (ft.value * fs.value)).withUnit[Kilogram / (Meter ^ 3)]

    // Section "4.7.2", "Flue gas density"

    override lazy val ρ_G_calc =
        (ft, fs) => (1.282 / (ft.value * fs.value)).withUnit[Kilogram / (Meter ^ 3)]
    ...
    override final def t_outside_air_mean = 0.degreesCelsius
    ...
    override lazy val p_h_calc =
        (h, ρL, ρG) => (G.value * h.value * (ρL.value - ρG.value)).pascals
    ...
    override lazy val p_R_calc =
        (λf, pd, l, dh) => (λf.value * pd.value * (l / dh).value).pascals

    // Section "4.9.4.2", "Dynamic Pressure (p_d)"

    override lazy val p_d_calc = (ρ, v) => (ρ.value * math.pow(v.value, 2) / 2.0).pascals
    ...
    override final val fluegas_σ_CO2_calc: σ_CO2 = 7.05.percent

    /** teneur en eau du bois (% en masse) */
    override final val wood_σ_H2O_calc: σ_H2O = 17.0.percent

    override final val η_calc =
        (tf) =>
    ...
    // 4.9.5
    override final def ζ1_modified_calc(
        ζα1: ζ,
        ζα2: ζ,
        ζα3: ζ,
        α1 : QtyD[Degree],
    ...
    override final def ζ2_modified_calc(
        ζα1: ζ,
        ζα2: ζ,
        ζα3: ζ,
        α1 : QtyD[Degree],
