...
open class `EN13384_1_A1_2019_Formulas` extends EN13384_1_A1_2019_Formulas_Alg:
    ...
    def thermal_resistance_for_layer_calc(
        ...
        SquareMeterKelvinPerWatt(
            cof.unwrap * dhi.to_m.value / (2.0 * lambda.toUnit[Watt / (Meter * Kelvin)].value) * math.log(
                (dho / dhi).value
            )
        )
                        ...
                        l match
                            case AppendLayerDescr.FromLambda(og, lambda)                     =>
                                computeTR(dhi, og, lambda).map: tr =>
                                    debug(
                                        s"Rth = ${tr.show} (${dhi.show}, ${og.show}, ${lambda.show}) [ig = ${ig.show} & og = ${og.show}]"
                                    )
                            ...
                            case AppendLayerDescr.FromLambdaUsingThickness(e, lambda)        =>
                                ...
                                computeTR(dhi, og, lambda).map: tr =>
                                    debug(
                                        s"Rth = ${tr.show} (${dhi.show}, ${e.show}, ${lambda.show}) [ig = ${ig.show} & og = ${og.show}]"
                                    )
                                    (og, trdhsum + tr / dhi, Status.Ok)
                            ...
                            case AppendLayerDescr.AirSpaceUsingOuterShape(osh, _, _)         =>
                                ...
                                deadAirSpaceThermalResistance(t_emittingSurfaceTemp, dn, ig).map: tr =>
                                    debug(
                                        s"Rth = ${tr.show} (air space: ${mean_gas_temp.show}, ${dn.show}) [ig = ${ig.show} & outer = ${osh.show}]"
                                    )
                            ...
                            case AppendLayerDescr.AirSpaceUsingThickness(e, _, _)            =>
                                ...
                                deadAirSpaceThermalResistance(t_emittingSurfaceTemp, e, ig).map: tr =>
                                    debug(
                                        s"Rth = ${tr.show} (air space: ${mean_gas_temp.show}, ${e.show}) [ig = ${ig.show} & osh = ${osh.show}]"
                                    )
                                    (osh, trdhsum + tr / dhi, Status.Error_MissingLayerAfterAirSpace)
        finalAcc
            .map(_._3)
    ...
    def T_u_calc(
        ...
        val num =
            (T_ub * A_ub) + (T_uh * A_uh) + (T_uu * A_uu) + (T_ul * A_ul) + (T_u_custom_area.getOrElse(
                0.degreesKelvin
            ) * A_u_custom_area)
    ...
    def T_u_temp_helper(
        ...
        extension (e: Either[UseTuoOverride, TCelsius])
            def getOrUseTuo: TKelvin =
                e
                    .leftMap(_ =>
                        T_uo_calc(T_L, unheatedHeightInsideAndOutside, airSpace, pReq, flueGasCond)(using T_uo_override)
                    )
    ...
    override final val T_uo_default = T_uo_temperature.from(
    ...
    def openedAirSpaceThermalResistance(
        t_emittingSurfaceTemp: TCelsius,
        dn_airSpaceWidth     : Length
    ): SquareMeterKelvinPerWatt =
