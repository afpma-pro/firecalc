...
object circe:
    ...
    given Decoder[BillingLanguage] =
        Decoder[String].emap(s => BillingLanguage.values.find(_.code == s).toRight(s"Invalid billing language: $s"))

    // Import version types and their Circe givens from companion objects
    import afpma.firecalc.ui.models.schema.common.{BillingInfo_Version, ClientProjectData_Version}
    import afpma.firecalc.ui.models.schema.common.BillingInfo_Version.{
        given Encoder[BillingInfo_Version],
        given Decoder[BillingInfo_Version]
    }
    import afpma.firecalc.ui.models.schema.common.ClientProjectData_Version.{
        given Encoder[ClientProjectData_Version],
        given Decoder[ClientProjectData_Version]
    }
