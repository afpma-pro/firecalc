...
object CatalogState:
    val empty: CatalogState =
        CatalogState(ListMap.empty, ListMap.empty, ListMap.empty, ListMap.empty, ListMap.empty, ListMap.empty)
    ...
    def merge(current: CatalogState, file: CatalogFile): CatalogState =
        ...
        val newFireboxes            = ListMap.from(
            file.entriesFor[Firebox.Door15aFirebox_Catalog]
                .map(e => e.reference -> e)
        )
        val newSingleTested         = ListMap.from(
            file.entriesFor[Firebox.SingleTested]
                .map(e => e.reference -> e)
        )
        val newPresets              = ListMap.from(
            file.entriesFor[SetThermalPipeProp_13384.SetPropertiesInBatch]
                .map(e => e.batch_name -> e)
        )
        val newCasings              = ListMap.from(
            file.entriesFor[CasingPreset]
                .map(e => e.unwrap.batch_name -> e.unwrap)
        )
        val newFlowResistances      = ListMap.from(
            file.entriesFor[FlowResistanceCatalogEntry]
                .map(e => e.name -> e)
        )
        val newAnglePresets         = ListMap.from(
            file.entriesFor[AnglePresetCatalogEntry]
                .map(e => e.reference -> e)
        )
        CatalogState(
...
object CatalogStateCodec:
    ...
    private given Encoder[Firebox.Door15aFirebox_Catalog] =
        semiauto
            .deriveEncoder[Firebox.Door15aFirebox_Catalog]
    ...
    private given Encoder[SetThermalPipeProp_13384.SetPropertiesInBatch] =
        semiauto
            .deriveEncoder[SetThermalPipeProp_13384.SetPropertiesInBatch]
            .mapJson(_.mapObject(_.remove("image")))

    given Decoder[CatalogState] = Decoder.instance { c =>
        for
            fireboxes       <- c
                .downField("door_15a_fireboxes")
                .as[Option[ListMap[String, Firebox.Door15aFirebox_Catalog]]]
                .map(_.getOrElse(ListMap.empty))
            singleTested    <- c
                .downField("single_tested_fireboxes")
                .as[Option[ListMap[String, Firebox.SingleTested]]]
                .map(_.getOrElse(ListMap.empty))
            presets         <- c
                .downField("pipe_presets")
                .as[Option[ListMap[String, SetThermalPipeProp_13384.SetPropertiesInBatch]]]
                .map(_.getOrElse(ListMap.empty))
            casings         <- c
                .downField("casing_presets")
                .as[Option[ListMap[String, SetThermalPipeProp_13384.SetPropertiesInBatch]]]
                .map(_.getOrElse(ListMap.empty))
            flowResistances <- c
                .downField("flow_resistance_presets")
                .as[Option[ListMap[String, FlowResistanceCatalogEntry]]]
                .map(_.getOrElse(ListMap.empty))
            anglePresets    <- c
                .downField("angle_presets")
                .as[Option[ListMap[String, AnglePresetCatalogEntry]]]
                .map(_.getOrElse(ListMap.empty))
