...
object PaymentsBackendApiConnectivity:
    ...
    case class BackendCheckInProgress(internetStatus: InternetConnected, backendStatus: BackendChecking)
        extends ConnectivityResult
    case class PartialConnection(internetStatus: InternetConnected, backendStatus: BackendDisconnected)
        extends ConnectivityResult
    case class FullConnection(internetStatus: InternetConnected, backendStatus: BackendConnected)
        extends ConnectivityResult
    ...
    def checkHttpStatus200(
        ...
        FetchStream
            ...
            .map {
                ...
                case Failure(error)       =>
                    Left(NetworkError(I18N_UI.connectivity_status.network_request_failed.apply(error.getMessage)))
            }
     ...
     */
    def performBackendHealthCheck(
        url: String = UIConfig.Endpoints.healthcheck
    )(using Locale): EventStream[BackendStatus] =
     ...
     */
    def performFullConnectivityCheck(
        url: String = UIConfig.Endpoints.healthcheck
    )(using Locale): EventStream[ConnectivityResult] =
