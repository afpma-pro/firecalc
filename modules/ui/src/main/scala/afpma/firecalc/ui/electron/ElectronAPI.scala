...
object ElectronAPI:
    ...
    def isAvailable: Boolean =
        try js.typeOf(js.Dynamic.global.window.electronAPI) != "undefined"
    ...
    def readFile(filePath: String)(using Locale): Future[Either[String, String]] =
        ...
        ElectronAPIImpl
            ...
            .recover { case ex: Throwable =>
                Left(I18N_UI.errors.failed_to_read_file.apply(ex.getMessage))
            }
    ...
    def writeFile(filePath: String, content: String)(using Locale): Future[Either[String, Unit]] =
        ...
        ElectronAPIImpl
            ...
            .recover { case ex: Throwable =>
                Left(I18N_UI.errors.failed_to_write_file.apply(ex.getMessage))
            }
    ...
    def openFileDialog()(using Locale): Future[Either[String, Option[String]]] =
        ...
        ElectronAPIImpl
            ...
            .toFuture
            .map { result =>
                if result.canceled.getOrElse(false                                               ) then Right(None)
            ...
            .recover { case ex: Throwable =>
                Left(I18N_UI.errors.failed_to_open_dialog.apply(ex.getMessage))
            }
    ...
    def saveFileDialog(defaultFileName: String)(using Locale): Future[Either[String, Option[String]]] =
        ...
        ElectronAPIImpl
            ...
            .recover { case ex: Throwable =>
                Left(I18N_UI.errors.failed_to_save_dialog.apply(ex.getMessage))
            }
