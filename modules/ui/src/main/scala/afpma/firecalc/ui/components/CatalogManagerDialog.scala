                ...
                ul(
                    ...
                    li(
                        a   (
                            ...
                            target := "_blank",
                            I18N_UI.catalog.afpma_catalog_page
                        )
                    )
                )
            ),
            ...
            div(
                ...
                div(
                    ...
                    child <-- catalogStateVar.signal.map: state =>
                        val hasEntries =
                            state.door_15a_fireboxes.nonEmpty || state.single_tested_fireboxes.nonEmpty || state.pipe_presets.nonEmpty || state.casing_presets.nonEmpty || state.flow_resistance_presets.nonEmpty || state.angle_presets.nonEmpty
                        if !hasEntries then p     (cls := "text-base-content/50 italic", I18N_UI.catalog.no_catalog_loaded)
                        else
                            div                   (
                                ...
                                Option.when(state.single_tested_fireboxes.nonEmpty)(
                                    p(
                                        s"• ${I18N_UI.catalog.single_tested_fireboxes}: ${state.single_tested_fireboxes.size}"
                                    )
                                ),
                                ...
                                Option.when(state.flow_resistance_presets.nonEmpty)(
                                    p(
                                        s"• ${I18N_UI.catalog.flow_resistance_presets}: ${state.flow_resistance_presets.size}"
                                    )
                                ),
