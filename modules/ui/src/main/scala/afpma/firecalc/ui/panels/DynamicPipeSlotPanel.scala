...
object DynamicPipeSlotPanel:
    ...
    def forSlot(slotIndex: Int, slot: PostFireboxPipeDescrSlot)(using Locale, DisplayUnits): PipePanel =
        slot match
            case PostFireboxPipeDescrSlot.FlueSlot(_)        =>
                DynamicFlowOnlyPipeSlotPanel(slotIndex)
            case PostFireboxPipeDescrSlot.ThermalFlueSlot(_) =>
                DynamicThermalPipeSlotPanel(slotIndex, FluePipeT, I18N.panels.channel_pipe)
            case PostFireboxPipeDescrSlot.ConnectorSlot(_)   =>
                DynamicThermalPipeSlotPanel(slotIndex, ConnectorPipeT, I18N.panels.connector_pipe)
            case PostFireboxPipeDescrSlot.ChimneySlot(_)     =>
                DynamicThermalPipeSlotPanel(slotIndex, ChimneyPipeT, I18N.panels.chimney_pipe)
...
final case class DynamicFlowOnlyPipeSlotPanel(slotIndex: Int)(using Locale, DisplayUnits)
    extends PipePanel:
    ...
    lazy val rendered_elems_sig: Signal[Seq[HtmlElement]] =
        welem_xtraoutput_sig
            ...
            .handleCase[(Int, FlowOnlyPipeDescr_15544, XtraOutputs), (Int, SetNumberOfFlows, XtraOutputs), HtmlElement] {
                ...
                renderElemTyped[SetNumberOfFlows](iix._1, I18N.set_prop.SetNumberOfFlows, iix._2, sig, isProperty = true)
            }
