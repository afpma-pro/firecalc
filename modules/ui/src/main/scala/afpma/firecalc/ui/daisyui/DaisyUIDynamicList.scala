...
trait DaisyUIDynamicList extends Component:
    ...
    lazy val welems_var: Var[Seq[(Int, A)]] = elems_v.bimap(xs => xs.mapWithIndex((x, i) => (i, x)))(_.map(_._2))
    ...
    def renderIdWithIncrDescr[AA <: A](
        idx             : Int,
        @nowarn welem   : (Int, AA),
        sig             : Signal[(Int, AA, XtraOutputs)],
        fullChildNode   : HtmlElement,
        summaryChildNode: Option[HtmlElement]
    ): HtmlElement =
        ...
        div(
            ...
            div(
                ...
                onClick(_.withCurrentValueOf(sig).collect: (_, id, a, _) =>
                    CollectionCommand.Insert((id, a), id + 1)) --> command_bus
            ),
            // remove button
            div(
                cls := "flex-none flex items-center text-base-content hover:bg-secondary hover:text-secondary-content justify-center cursor-pointer w-6 h-6",
            ...
            div(
                ...
                when(idx > 0)(
                    ...
                    onClick(_.withCurrentValueOf(sig)) --> Observer[(MouseEvent, Int, AA, XtraOutputs)] {
                        (_, id, a, _) =>
    ...
    def renderContent: HtmlElement =
        div(cls := "py-4 gap-2", div(cls := "flex flex-col gap-2", children <-- rendered_elems_sig))
