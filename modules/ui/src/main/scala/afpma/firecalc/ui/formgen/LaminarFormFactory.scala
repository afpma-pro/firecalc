...
trait LaminarFormFactory[DF[x] <: LaminarForm[x, DF[x]]] extends LaminarFormFactory.SplitAsSelectWithOptionsImpl[DF]:
    ...
    final def forOptionString(
        ...
        makeForOptionUsingCommonFactory(given_Defaultable_String)(optionStringFactory)(updateFieldName, optionalField)
    ...
    final def forOptionDouble(
        ...
        makeForOptionUsingCommonFactory(given_Defaultable_Double)(optionDoubleFactory)(updateFieldName, optionalField)
    ...
    final def forOptionLocalDate(
        updateFieldName: Option[String] => Option[String] = identity,
        optionalField  : OptionalField                    = OptionalField.No
    )                                     : VVOpt_to_DFOpt[LocalDate] = makeForOptionUsingCommonFactory(given_Defaultable_LocalDate)(optionLocalDateFactory)(
        updateFieldName,
        optionalField
    )
    ...
    final def forOptionNumeric[A: Numeric](
        ...
        def od2oa = (od: Option[Double]) => od.flatMap(d => Numeric[A].parseString(d.toString))
    ...
    extension [A](df: DF[A])
        def autoOverwriteFieldNames(using
            tc: HasTranslatedFieldsWithValues[A]
        ): DF[A] =
