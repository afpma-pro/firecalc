...
object PaymentLinkCreationTest extends TestSuite {
    ...
    def createPaymentLinkMockServices(repos: TestPaymentLinkRepositories) = {
        ...
        val emailService = new EmailService[IO] {
            def sendUserAuthenticationCode(authCode: AuthenticationCodeEmail)(using
                language: BackendCompatibleLanguage
            ): IO[email.EmailResult] =
                IO.pure(EmailSent)

            def sendUserInvoice(invoice: InvoiceEmail)(using
                language: BackendCompatibleLanguage
            ): IO[email.EmailResult] =
                IO.pure(EmailSent)
            ...
            def sendUserPaymentLink(
                paymentLink: PaymentLinkEmail
            )(using language: BackendCompatibleLanguage): IO[email.EmailResult] = IO.delay {
        ...
        val paymentService = new PaymentService[IO] {
            def createPaymentLink(orderId: OrderId, amount: BigDecimal, customerInfo: CustomerInfo): IO[String] =
                IO.delay {
                    repos.paymentLinkCreationAttempts =
                        (orderId, amount, customerInfo) :: repos.paymentLinkCreationAttempts
    ...
    val tests = Tests {
        ...
        test("payment link creation - handles failures with proper exception") {
            ...
            repos.paymentLinkErrors =
                repos.paymentLinkErrors + (order.id -> "class scala.Tuple11 cannot be cast to class scala.Tuple13")

            // Test payment link creation failure
            val result = paymentService
                .createPaymentLink(
                    order.id,
                    order.amount,
                    testCustomerInfo
        ...
        test("payment link creation - validates tuple handling across different customer types") {
            ...
            val individualAttempt =
                repos.paymentLinkCreationAttempts.find(_._3.customerType == CustomerType.Individual).get
