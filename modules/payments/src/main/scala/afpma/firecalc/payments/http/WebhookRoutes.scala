...
class WebhookRoutes[F[_]: Async](
    ...
    val routes_V1: HttpRoutes[F] = HttpRoutes.of[F] { case req @ POST -> Root / "v1" / "webhooks" / "gocardless" =>
                ...
                (
                    for
                        ...
                        _ <-
                            ...
                            then Async[F].raiseError(WebhookSignatureInvalidException(signature))
                            else Async[F].unit
