...
class MoleculePurchaseIntentRepository[F[_]: Async: Logger](using conn: Conn, ec: ExecutionContext)
    extends PurchaseIntentRepository[F]:
    ...
    def create(
        ...
        for
            ...
            intent                <- createWithInternalCustomerId(
                productId,
                amount,
                currency,
                authCode,
    ...
    def deleteExpired(): F[Int] =
        ...
        for
            ...
            idsToDelete <- future2AsyncF {
                PurchaseIntent.id.token.expiresAt.query.get
                    .map(
                        _.filter(_._3 < now)
                            .map(_._1)
                    )
