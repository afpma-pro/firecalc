...
class MoleculeCustomerRepository[F[_]: Async: Logger](using conn: Conn, ec: ExecutionContext)
    ...
    def create(customerInfo: CustomerInfo): F[domain.Customer] =
        for
            ...
            _ <- future2AsyncF {
                Customer.customerId.email.customerType.language.givenName.familyName.companyName.addressLine1.addressLine2.addressLine3.city.region.postalCode.countryCode.phoneNumber.paymentProviderId.paymentProvider.createdAt.updatedAt.insert {
                    import customerInfo.*
                    (
                        customerId,
                        email,
                        ...
                        countryCode.map(_.code).getOrElse(""),
                        phoneNumber.getOrElse            (""),
                        "",
                        PaymentProvider.Unknown.toString,
                        now,
                        now
                    )
                }.transact
            ...
            _ <- future2AsyncF {
                Customer.customerId.email.customerType.language.givenName.familyName.companyName.addressLine1.addressLine2.addressLine3.city.region.postalCode.countryCode.phoneNumber.paymentProviderId.paymentProvider.createdAt.updatedAt.insert {
                    ...
                    (
                        ...
                        countryCode.map(_.code).getOrElse(""),
                        phoneNumber.getOrElse            (""),
                        paymentProviderId.getOrElse      (""),
                        unsafePaymentProvider.getOrElse  (""),
                        now,
                        now
                    )
                }.transact
    ...
    def updatePaymentProvider(
        ...
        for
            _                  <- logger.info(
                s"Updating customer ${customerId.value} with payment provider: $paymentProvider, ID: $paymentProviderId"
            )
            now                <- Async[F].delay(Instant.now())
            ...
            result             <- future2AsyncF {
                Customer(customerInternalId)
                    ...
                    .paymentProvider(PaymentProvider.valueOf(paymentProvider.toString))
                    .updatedAt(now)
                    .update
                    .transact
            }
        ...
        for
            ...
            result           <- existingCustomer match
                case Some(customer) =>
                    for
                        now                <- Async[F].delay(Instant.now())
                        customerInternalId <- future2AsyncF {
                            Customer.id.customerId_(customer.id.value).query.get.map(_.head)
                        }
                        _                  <- future2AsyncF {
                            Customer(customerInternalId)
                                .customerType(
                                    afpma.firecalc.payments.repository.impl.dsl.MoleculeDomain.CustomerType.valueOf(
                                        newCustomerInfo.customerType.toString
                                    )
                                )
