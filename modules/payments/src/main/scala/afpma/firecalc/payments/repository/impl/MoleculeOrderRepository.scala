...
import afpma.firecalc.payments.shared.api.ProductId
...
class MoleculeOrderRepository[F[_]: Async: Logger](using conn: Conn, ec: ExecutionContext) extends OrderRepository[F]:
                        ...
                        ProductOrder.orderId.customerId.productId.amount.currency.status.paymentProvider.paymentId.language.productMetadata.createdAt.updatedAt
                            .insert(
                                (
                                    ...
                                    currency.toString,
                                    "Pending",
                                    paymentProvider.map(_.toString).getOrElse(""),
                                    paymentId.getOrElse                      (""),
                                    language.code,
                                    productMetadataId,
                                    now,
                            ...
                            .map:
                                case (
                                    ...
                                    domain.ProductOrder               (
                                        ...
                                        paymentProvider   =
                                            if paymentProviderStr.isEmpty then None
                                            else Some(domain.PaymentProvider.valueOf(paymentProviderStr)),
        ...
        for
            ...
            moleculePaymentProvider <- Try(PaymentProvider.valueOf(paymentProvider.toString)) match {
                ...
                case scala.util.Failure(ex)       =>
                    Async[F].raiseError(
                        new IllegalArgumentException(s"Invalid payment provider: ${paymentProvider.toString}", ex)
                    )
            }
            internalId              <- future2AsyncF {
                ProductOrder.id.orderId_(id.value).query.get.map(_.head)
            ...
            result                  <- future2AsyncF {
                ProductOrder(internalId)
                    .paymentId(paymentId)
                    .paymentProvider(moleculePaymentProvider)
                    .updatedAt(now)
                    .upsert
                    .transact
            }
    ...
    def findConfirmedOrdersWithoutInvoiceNumber(): F[List[domain.ProductOrder]] =
        for
            ...
            result <- future2AsyncF {
                ProductOrder.orderId.customerId.productId.amount.currency
