...
object EmailAddress:
    ...
    def fromString(email: String): Either[String, EmailAddress] =
        if email.trim.isEmpty then Left("Email address cannot be empty")
        else if email.contains("@@") then Left("Email address contains double @ character")
        else if EMAIL_REGEX.matches(email.trim                     ) then Right(email.trim)
