...
import afpma.firecalc.payments.shared.api.{
    OrderId,
    CustomerType,
    BackendCompatibleLanguage,
    CountryCode_ISO_3166_1_ALPHA_2,
    CustomerInfo,
    ProductId,
    PurchaseToken
}
...
object Codecs:
    ...
    given Decoder[OrderStatus] =
        Decoder[String].emap(s => OrderStatus.values.find(_.toString == s).toRight(s"Invalid status: $s"))

    given Encoder[PaymentProvider] = Encoder[String].contramap(_.toString)
    given Decoder[PaymentProvider] =
        Decoder[String].emap(s => PaymentProvider.values.find(_.toString == s).toRight(s"Invalid payment provider: $s"))

    given Encoder[Currency] = Encoder[String].contramap(_.toString)
    given Decoder[Currency] =
        Decoder[String].emap(s => Currency.values.find(_.toString == s).toRight(s"Invalid currency: $s"))
