                            ...
                            order.paymentId match
                                case Some(existingId) =>
                                    if (existingId == pId) Async[F].pure(true)
                                        ...
                                        Async[F].raiseError(
                                            PaymentIdMismatchException(orderId.value.toString, existingId, pId)
                                        )
                                case None             =>
...
//   }
    ...
    private def executeTransitionCallbacks(transition: OrderStateTransition, order: ProductOrder): F[Unit] =
        for
            ...
            _ <- logger.info(
                s"Executing ${triggeredCallbacks.size} callbacks for transition ${transition.from} -> ${transition.to} on order ${order.id}"
            )
            _ <- triggeredCallbacks.traverse_ { transitionCallback =>
                Async[F].start(
                    (for
                        _ <- logger.info(
                            s"Executing transition callback: ${transitionCallback.name} for order ${order.id}"
                        )
                        _ <- transitionCallback.callback(context)
                        _ <- logger.info(
                            s"Transition callback '${transitionCallback.name}' executed successfully for order ${order.id}"
                        )
                    yield ()).handleErrorWith(error =>
                        logger.error(
                            s"Transition callback '${transitionCallback.name}' failed for order ${order.id}: ${error.getMessage}"
                        )
                    )
                )
            }
    ...
    private def executeFinalStateCallbacks(order: ProductOrder): F[Unit] =
        for
            ...
            _ <- logger.info(
                s"Executing ${pendingCallbacks.size} final state callbacks for status ${order.status} on order ${order.id}"
            )
            _ <- pendingCallbacks.traverse_ { finalCallback =>
                Async[F].start(
                    (for
                        ...
                        _ <- logger.info(
                            s"Final state callback '${finalCallback.name}' executed successfully for order ${order.id}"
                        )
                    ...
                    yield ()).handleErrorWith(error =>
                        logger.error(
                            s"Final state callback '${finalCallback.name}' failed for order ${order.id}: ${error.getMessage}"
                        )
                    )
                )
            }
