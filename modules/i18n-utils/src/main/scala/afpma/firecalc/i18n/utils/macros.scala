...
object macros:
    ...
    private def getTranslatedFieldsWithValuesImpl[T: Type, I18nType: Type](i18nData: Expr[I18nType])(using
        q: Quotes
    ): Expr[TranslatedFieldsWithValues] =
    ...
    private def accessPathImpl[T: Type, R: Type](
        ...
        path.value.getOrElse(
            report.errorAndAbort(
                s"Path must be a compile-time constant, but got :\n${path.asTerm.show(using Printer.TreeCode)}"
            )
        ) match
            case fieldPath =>
                try
        ...
        def extractDefDef(tree: Term): List[String] =
            tree match
                ...
                case rem                                                                            =>
                    // report.warning(s"AST:\n${rem.show(using Printer.TreeStructure)}")
                    report.errorAndAbort(
                        s"Unexpected tree structure (defdef): ${rem.show(using Printer.TreeStructure)}",
                        expr
                    )
                    Nil
        expr match
